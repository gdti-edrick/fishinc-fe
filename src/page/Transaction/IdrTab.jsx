...
import {
  useTransactionListIdr,
  useTransactionSummaryIdr,
  useTransactionSummaryIdrItem,
...
const IdrTab = () => {
  ...
  const { data: dataTransactionListAll, isLoading } =
    useTransactionListIdr(queryParams);

  const { data: dataTransactionSummaryAll } =
    useTransactionSummaryIdr(queryParams);
  const { data: dataTransactionSummaryAllItem } =
    useTransactionSummaryIdrItem(queryParams);
  ...
  return (
    <Box sx={{ padding: "16px" }}>
      ...
      <ListAllTable
        ...
        doDownload={() =>
          downloadExcel(dataTransactionListAll?.data, "transaction_list_idr")
        }
        start={startDate}
